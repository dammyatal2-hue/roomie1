import { ArrowLeft, Share, Heart, Home, Bed, Bath, Sofa, DollarSign, Calendar, MapPin, User, Hospital, ShoppingCart, ShoppingBag, Fuel, Bus, GraduationCap, Dumbbell, Plus, Cross, Coffee, X } from "lucide-react";
...
export function PropertyDetails({ onBack, listing, onRequestToJoin, onViewProfile, record }: PropertyDetailsProps) {
  const { user } = useAuth();
          ...
          <div className="px-[24px] pt-[32px]">
            ...
            <div className="bg-white border border-[#e5e7eb] rounded-[12px] p-[16px] shadow-[0px_1px_2px_rgba(0,0,0,0.05)]">
              <div className="flex items-center gap-[12px] mb-[12px]">
                {/* Avatar */}
                {record?.profiles?.avatar_url ? <img src={record.profiles.avatar_url} alt={record.profiles.full_name || record.profiles.username || "Listing owner"} className="flex-none w-12 h-12 rounded-full object-cover" /> : <div className="flex-none w-[48px] h-[48px] rounded-full bg-gradient-to-br from-[#fe456a] to-[#ff758f] flex items-center justify-center"><User className="w-[24px] h-[24px] text-white" strokeWidth={2} /></div>}

                {/* Info */}
                <div className="flex-1 min-w-0">
                  <h3 className="font-['Inter:Semi_Bold',sans-serif] font-semibold text-[15px] leading-[20px] text-[#1f2a37] mb-[2px]">
                    {record?.profiles?.full_name || record?.profiles?.username || "Roomie member"}
                  </h3>
                  <p className="font-['Inter:Regular',sans-serif] font-normal text-[13px] leading-[18px] text-[#6b7280]">
                    {[record?.profiles?.occupation, record?.profiles?.city].filter(Boolean).join(" · ") || "Listing Owner"}
                  </p>
                </div>
                {record?.owner_id && <button type="button" onClick={() => onViewProfile?.(record.owner_id)} className="text-xs font-medium text-[#fe456a]">View profile</button>}
              </div>
